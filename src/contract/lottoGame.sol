...
function joinPool(uint poolId,uint amount) external onlyNewPartipant(poolId,msg.sender)  returns(bool){
    _timecheck(poolId,block.timestamp);
    LottoPool memory pool=poolById[poolId];
...
return (poolId,poolById[poolId]._participantcounter,poolById[poolId]._maxParticipants,poolById[poolId]._minimumContribution,poolById[poolId]._timestamp.add(duration));
}
